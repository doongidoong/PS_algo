...
- 풀이접근: 
<p>
DFS를 활용했다. 부등호를 기준으로 1,10까지 수를 모두 넣어보며 만약 이미 사용한 수라면 pass, 사용하지 않으면 이를 넣고 level을 올린다.<br>
...
- 풀이접근: 
<p>
만약 3,4,2,1이 있다고 보자, 여기서 교환해야하는 값은 바로 3이다. 왜냐하면 4,2,1의 경우 내림차순으로 정렬되어 있으므로
<br>4,1,2 / 2,4,1 등등 어떤 조합을 해도 다음 순열을 얻을 수 없다. 그러므로 3을 교환해야 한다.   
<br>하지만 여기서 또 중요한 점은 3보다 큰 수인 4와 교환해야만 더 큰 순열을 얻을 수 있다. <br>
이를 만족한다면 이제 바로 다음이라는 조건을 만족해야 하므로 나머지 수는 모두 정렬한다.
<br>그러므로 간단히 요약하자면, 뒤에서부터 오름차순이 아닌 수를 찾는다. 그 수보다 크지만 가장 작은 숫자와 교환한다. 나머지 숫자는 정렬하여 합친다.  

- 어려웠던 점:
시간초과를 해결하지 못했었다. 브루트포스라는 생각에 완전탐색을 생각했지만 해답은 그리디에 조금 더 가까운 것 같다. 하지만 풀이법을 생각하지 못했으니 다음에 꼭 기억하고 풀어보자
...
- 풀이접근: 
 <p>
부분 수열을 만들 때, 이진수의 & 연산을 통해 경우의 수를 표현했다. <br>
 만약 집합의 크기가 5라면 모든 경우를 고려하면 공집합을 제외한 00001 ~ 11111까지이다. 여기서 on, 비트가 1인 경우는 해당 원소를 사용한다는 의미이다.<br> 즉 1에서 32까지 반복문을 돌리면서, 00001, 00010 이런 식으로 각 자리 수와 & 비교 연산을 수행하면 해당 집합을 사용하는 경우인 지 비교할 수 있다. 만약 6이라면, 00110이므로 두 번째, 세 번째 원소를 더해주면 된다.   

- 어려웠던 점:
비트마스크를 아직까지 잘 이해하지 못한 것 같다. 비트 연산의 결과 값을 머릿 속으로 이해하는 것이 중요한 것 같다.  
...
- 풀이접근: 
 <p>
각 연산별 풀이는 다음과 같다. 
 <br>add: 해당 원소만큼 shift한 값과 or 연산을 해준다.
 <br>remove: 해당 원소만큼 shift한 값을 뒤집은 것과 and 연산을 해준다.
 <br>check: 해당 비트가 1이라면 1을 출력한다.
 <br>toggle: xor 연산을 해준다.
 <br>all : bit = (1<< 20 ) -1 로 비트를 바꾼다
 <br>empty: 0으로 바꿔준다
 
 
- 어려웠던 점:
비트 연산을 이해하는 좋은 문제가 되었다.
...
- 풀이접근: 
 <p>
연산자의 개수가 주어진다. 이런 연산자는 한번씩 사용할 수 있다.
따라서 함수에 각 연산자의 현재 사용한 개수를 인자로 받게하여 연산자 개수를 비교하도록 하면 된다.
 <br>
 
- 어려웠던 점:
사실 이전에 비슷한 문제는 풀어보아서 큰 어려움은 없었다.
...
- 풀이접근: 
 <p>
동시에 움직이므로 BFS에 담을 때, 두 점을 한번에 담는게 좋다. 또한 visit을 체크할 때, 중요한 점은 두 점이 해당 위치를 방문했을 경우의 조합도 생각해야 한다. 예를 들어 (1,1) (0,0)과 (1, 1) (1,1)은 첫 번째 스팟을 기준으로는 같은 점을 방문했으나 조합을 생각하면 같은 경우가 아니다. 따라서 이를 표현하기 위해 4차원 배열을 사용했다. 그리고 벽이 있는 경우, 다시 돌아가는 연산으로   
 <br>
 
- 어려웠던 점:
아이디어는 금방 떠올렸으나 구현에 어려움이 있었다.
